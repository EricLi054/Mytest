...
[TestFixture]
public class FinOpsMapperTests
{
    private Mock<ILogger<FinOpsMapper>> LoggerMock = new();
    private FinOpsMapper _finOpsMapper = null!;

    [SetUp]
    public void SetUp()
    {
        _finOpsMapper = new(LoggerMock.Object);
    }
    ...
    public void Map_ShouldReturnNullAndLogError_WhenBusinessTypeIsNotRSA()
    {
        ...
        var result = _finOpsMapper.Map(product);

        Assert.That(result, Is.Null);
        LoggerMock.VerifyLog(LogLevel.Error, "Not supported product type NotRSA", Times.Once);
    }
    ...
    public void Map_ShouldReturnNullAndLogError_WhenProductIsNotRoadsideProductHolding()
    {
        ...
        var result = _finOpsMapper.Map(product);
        Assert.That(result, Is.Null);
        LoggerMock.VerifyLog(LogLevel.Error, "Unexpected product type AnnuityProduct", Times.Once);
    }
    ...
    public void Map_ReturnsCorrectPolicyDetail_WhenValidRoadsideProduct()
    {
        ...
        // Act
        var result = _finOpsMapper.Map(roadsideProduct);
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(result?.PolicyItems, Has.Count.EqualTo(4));
    ...
    public void MapPolicyItems_AddsBundledAmount_WhenProductIsBundled()
    {
        ...
        roadsideProduct.BundledProducts = [
            new Membership.Types.Products.AnnuityProducts.BundledProduct
            {
    ...
    public void Map_AddsBundledPayNowPolicyItems_WhenShowPayNowAndIsBundledAreTrue()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(bundledProducts, Is.Not.Null);
            Assert.That(bundledProducts, Is.Not.Empty);
            Assert.That(bundledProducts?.Any(bp => bp.ProductName == "Additional Vehicle Cover" &&
                                                    bp.Asset == "2023 Toyota Camry ABC123"), Is.True, "Bundled product 'Additional Vehicle Cover' should be correct.");

            Assert.That(bundledProducts?.Any(bp => bp.ProductName == "Trailer Cover" &&
                                                    bp.Asset == "2020 Honda CR-V XYZ789"), Is.True, "Bundled product 'Trailer Cover' should be correct.");
    ...
    [Test]
    public void Map_ShouldNotPopulateAlert_WhenProductCanUpdateVehicle()
    {
        // Arrange
        var roadsideProduct = CreateRoadsideProduct(new FinOpsProductFlags { CanUpdateVehicle = true });
        roadsideProduct.Type = FinOpsConstants.ProductCodes.StandardRSA;

        // Act
        var result = _finOpsMapper.Map(roadsideProduct);

        // Assert
        Assert.That(result?.Alerts, Is.Null.Or.Empty, "Alert should not be populated for product that Can Update Vehicle.");
    }

    [Test]
    public void Map_ShouldPopulateAlert_WhenProductCanNotUpdateVehicleAndIsNotRewards()
    {
        // Arrange
        var roadsideProduct = CreateRoadsideProduct(new FinOpsProductFlags { CanUpdateVehicle = false });
        roadsideProduct.Type = FinOpsConstants.ProductCodes.StandardRSA;
        ...
        // Assert
        Assert.That(result?.Alerts, Is.Not.Null.Or.Empty);
        Assert.That(result?.Alerts?.Count, Is.EqualTo(1), "Expected exactly one alert.");
    }

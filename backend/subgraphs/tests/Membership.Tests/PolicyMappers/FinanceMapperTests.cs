...
{
    private Mock<ILogger<FinanceMapper>> LoggerMock = new();
    private FinanceMapper _financeMapper = null!;

    [SetUp]
    public void SetUp()
    {
        _financeMapper = new(LoggerMock.Object);
    }
    ...
    public void Map_ShouldReturnNullAndLogError_WhenBusinessTypeIsNotFinance()
    {
        ...
        var result = _financeMapper.Map(product);

        Assert.That(result, Is.Null);
        LoggerMock.VerifyLog(LogLevel.Error, "Not supported product type NotFinance", Times.Once);
    }
    ...
    public void Map_ShouldMapFieldsCorrectly_WhenProductIsFinanceProductHolding()
    {
        ...
        var mappedPolicy = _financeMapper.Map(financeProduct);

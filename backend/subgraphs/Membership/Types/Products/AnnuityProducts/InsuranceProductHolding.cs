...
public class InsuranceProductHolding : AnnuityProduct
{
    ...
    {
        base.BusinessType = Products.BusinessType.Insurance.ToString();
        ...
        ConfigureMemberActions(policyPaymentInfo, configuration, featureFlags);
    }
    ...
    private void ConfigureMemberActions(InsuranceProductResponse? policyPaymentInfo, IConfiguration configuration, Dictionary<string, bool> featureFlags)
    {
        ...
        bool showUpdateHowYouPay = policyPaymentInfo != null &&
                                   policyPaymentInfo.PaymentMethod != InurancePaymentType.AnnualCash &&
                                   type switch
                                   {
                                       InsuranceType.PET => featureFlags.TryGetValue(FeatureFlags.UHYP_Pet, out var petFlag) && petFlag,
                                       InsuranceType.BGP => featureFlags.TryGetValue(FeatureFlags.UHYP_Boat, out var boatFlag) && boatFlag,
                                       InsuranceType.MGE => featureFlags.TryGetValue(FeatureFlags.UHYP_Mobility, out var mobilityFlag) && mobilityFlag,
                                       _ => true
                                   };
        ...
        // Urls
        string b2cUrl = configuration[ConfigurationKeys.InsuranceB2CUrl] ?? "";
        string pcmBase = $"{b2cUrl}/Secure/PCM";

        string managePolicyLink = $"{pcmBase}?policyNumber={PolicyNumber}";
        string updateHowYouPayLink = $"{configuration[ConfigurationKeys.InsuranceUpdateHowYouPayUrl]}?policyNumber={PolicyNumber}";
        ...
        string makeAClaimLink = type switch
        {
            ...
            InsuranceType.MGP => $"{b2cUrl}/claims/{Description}/what-are-you-claiming-for?policyNumber={PolicyNumber}",
            InsuranceType.HGP => $"{b2cUrl}/claims/{Description}/building-and-contents?policyNumber={PolicyNumber}",

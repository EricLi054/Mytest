...
public class RoadsideProductHolding : AnnuityProduct
{
    ...
    public List<CTALink>? Actions
    {
        get
        {
            ...
            {
                // Add View Membership / View Cover as the cta option
    ...
    public RoadsideProductHolding(FinOpsProductHolding productHoldingHeader, ProductHoldingLine productHoldingLine, FinOpsProductFlags productFlags)
    {
        ProductFlags = productFlags;
        ProductId = productHoldingLine.ProductId;
        Id = productHoldingLine.ProductHoldingId;
        base.BusinessType = Descriptors.BusinessType.RSA.ToString();
        HeaderId = productHoldingHeader.ProductHoldingHeaderId;
        Title = productHoldingLine.ProductName;
        Subtitle = PolicyCardTitle;
        Version = productHoldingLine.ProductHoldingVersion;
        Asset = new Vehicle { Year = productHoldingLine.VehicleDetail.Year, Make = productHoldingLine.VehicleDetail.Make, Model = productHoldingLine.VehicleDetail.Model };
        ...
        // You are on non direct debit IF the payment mode or renewal payment modes are empty
    ...
    private void ProcessPaymentInformation(FinOpsProductHolding productHoldingHeader)
    {
        var paymentDetail = productHoldingHeader.RenewalPaymentDetail ?? productHoldingHeader.PaymentDetail;

        if (paymentDetail == null)
        {
            // return gracefully if no payment detail is found, the front-end will handle the displaying of empty or undefined vals
            return;
        }

        // Determine the payment schedule and mode based on the renewal or the original product holding
        ...
        {
            var nextPayableInstallment = productHoldingPaymSched.Where(x => x.RemainingAmount > 0)
                .OrderBy(x => x.DueDate)
                .FirstOrDefault();

            // If the last installment is not paid, then we can display the next payment information
            if (nextPayableInstallment != null && ProductFlags.IsNotBundledOrFirstInBundle)
            {
                NextPayment = nextPayableInstallment.DueDate.ToString("dd MMM yyyy");
                NextPaymentAmount = nextPayableInstallment.Amount.ToString("0.00");
                ...
                NextPaymentActionDate = nextPayableInstallment.DueDate;
            }
        }

        // If the last installment is paid, just return gracefully, the front-end will handle the displaying of empty or undefined vals
    }
}

...
import { validateLandingPage } from './helpers/playwright.navigation';
...
test('GIVEN I am a Finance Loan Member, WHEN I log in, THEN I should see the Finance Loan Policy Card', async ({
  page
}: {
  page: Page;
}) => {
  await login(page, process.env.PLAYWRIGHT_FIN_LOAN_MEMBER, process.env.PLAYWRIGHT_FIN_LOAN_PASSWORD);
  await validateLandingPage(page);

...
const VWO = ({ accountId }: { accountId: string }) => {
  const [isRacDomain, setIsRacDomain] = useState(false)

  useEffect(() => {
    if (typeof window !== 'undefined') {
      if (
        window?.location.hostname.includes('ractest.com.au') ||
        window?.location.hostname.includes('rac.com.au')
      ) {
        setIsRacDomain(true)
      }
    }
  ...
  return (
    <>
      {isRacDomain && typeof window !== 'undefined' && (
        <>
          <link
            rel="preconnect"

...
export type UserType = "DEFAULT_USER" | "LANDLINE_ONLY_USER" | "NO_PHONE_USER" | "DIGITAL_CARD";
...
const envCredentials = {
  SIT: {
    username: process.env.PLAYWRIGHT_SIT_USERNAME ?? "",
    password: process.env.PLAYWRIGHT_SIT_PASSWORD ?? "",
  },
  UAT: {
    username: process.env.PLAYWRIGHT_UAT_USERNAME ?? "",
    password: process.env.PLAYWRIGHT_UAT_PASSWORD ?? "",
  },
};
...
export const specialCredentials = {
  digitalCard: envCredentials[getCurrentEnv()],
  activeDigitalCard: {
  ...
  default: envCredentials[getCurrentEnv()],
  finLoan: {
...
export function getCredentials(userType: UserType): Credentials {
  const credentialsMap = new Map<UserType, Credentials>([
    ["DEFAULT_USER", specialCredentials.default],
    ["DIGITAL_CARD", specialCredentials.digitalCard],
    ["LANDLINE_ONLY_USER", specialCredentials.landlineOnly],
    ["NO_PHONE_USER", specialCredentials.noPhone],
  ]);

  const credentials = credentialsMap.get(userType);
  if (!credentials) {

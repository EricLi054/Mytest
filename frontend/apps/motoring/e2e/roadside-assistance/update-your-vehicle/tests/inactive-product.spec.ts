...
const setup = async ({ account }: { account: MyRacAccount }) => {
  ...
  const classicProduct = await findDynamicsProduct({ productStatus: "Inactive", productType: "Classic" });
...
test("Inactive roadside product should be redirected to system-unavailable", async ({ page, account }) => {
  const { classicProduct } = await setup({ account });

  await page.goto(
    roadsideAssistanceUrl(
      `/update-your-vehicle?productHoldingHeaderId=${classicProduct.rac_productholdingheaderid}&productHoldingLineId=${classicProduct.rac_policynumber}`,
    ),
  );

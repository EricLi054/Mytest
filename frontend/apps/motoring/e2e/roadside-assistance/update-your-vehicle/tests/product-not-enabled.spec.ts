...
const setup = async ({ account }: { account: MyRacAccount }) => {
  ...
  const wheels2GoProduct = await findDynamicsProduct({ productStatus: "Active", productType: "Wheels2Go" });
  const linkResult = await linkPerson({ crmId: wheels2GoProduct._rac_personid_value, email: account.email });
...
test("Roadside product with vehicle update not enabled should be redirected to product-update-not-allowed", async ({
  page,
  account,
}) => {
  const { wheels2GoProduct } = await setup({ account });

  await page.goto(
    roadsideAssistanceUrl(
      `/update-your-vehicle?productHoldingHeaderId=${wheels2GoProduct.rac_productholdingheaderid}&productHoldingLineId=${wheels2GoProduct.rac_policynumber}`,
    ),
  );

...
describe("UpdateYourVehicleActions", () => {
  describe("createSession", () => {
    it("should return redirectTo /system-unavailable when productHoldingHeaderId is missing", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      vi.mocked(cookies).mockResolvedValue(mockRequestCookies);

      const { redirectTo } = await createSession({ ...defaultArgs, productHoldingHeaderId: undefined });

      expect(redirectTo).toBe("/system-unavailable");
    ...
    it("should return redirectTo /system-unavailable when productHoldingLineId is missing", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      vi.mocked(cookies).mockResolvedValue(mockRequestCookies);

      const { redirectTo } = await createSession({ ...defaultArgs, productHoldingLineId: undefined });
    ...
    it("should return redirectTo /system-unavailable when there is no CRM ID", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      vi.mocked(cookies).mockResolvedValue(mockRequestCookies);
      vi.mocked(getCrmId).mockReturnValue(Promise.resolve(undefined));

      const { redirectTo } = await createSession(defaultArgs);
    ...
    it("should return redirectTo /system-unavailable when there are errors querying roadside product data", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      vi.mocked(cookies).mockResolvedValue(mockRequestCookies);
    ...
    it("should return redirectTo /system-unavailable when personService is not alive", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      vi.mocked(cookies).mockResolvedValue(mockRequestCookies);
      vi.mocked(getCrmId).mockReturnValue(Promise.resolve("CRM ID"));
      vi.mocked(getRoadsideProductData).mockReturnValue(
    ...
    it("should return redirectTo /system-unavailable when vehicleService is not alive", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      vi.mocked(cookies).mockResolvedValue(mockRequestCookies);
      vi.mocked(getCrmId).mockReturnValue(Promise.resolve("CRM ID"));
      vi.mocked(getRoadsideProductData).mockReturnValue(
    ...
    it("should return redirectTo /system-unavailable when there is no member data", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
    ...
    it("should return redirectTo /system-unavailable when there is no roadside product", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      const mockSessionId = crypto.randomUUID();
      const mockCrmId = crypto.randomUUID();
      vi.mocked(cookies).mockResolvedValue(mockRequestCookies);
    ...
    it("should return redirectTo /system-unavailable when roadside product is not active", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      const mockSessionId = crypto.randomUUID();
      const mockCrmId = crypto.randomUUID();
      vi.mocked(cookies).mockResolvedValue(mockRequestCookies);
    ...
    it("should return redirectTo /system-unavailable when there is no roadside product line", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      const mockSessionId = crypto.randomUUID();
      const mockCrmId = crypto.randomUUID();
      const {
    ...
    it("should return redirectTo /change-already-made when canUpdateVehicle is false and reason is VEHICLE_CHANGE_LIMIT_REACHED", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      vi.mocked(cookies).mockResolvedValue(mockRequestCookies);
      vi.mocked(getCrmId).mockReturnValue(Promise.resolve("CRM ID"));
      vi.mocked(getRoadsideProductData).mockReturnValue(
      ...
      expect(redirectTo).toBe("/change-already-made");
      expect(mockRequestCookies.delete).toHaveBeenCalledTimes(1);
      expect(mockRequestCookies.delete).toHaveBeenCalledWith(uyvSessionIdCookieName);
      expect(createUpdateYourVehicleSession).not.toHaveBeenCalled();
      expect(mockRequestCookies.set).not.toHaveBeenCalled();
    ...
    it("should return redirectTo /product-update-not-allowed when canUpdateVehicle is false and reason is REGO_ONLY_CHANGE_ALLOWED", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      vi.mocked(cookies).mockResolvedValue(mockRequestCookies);
      vi.mocked(getCrmId).mockReturnValue(Promise.resolve("CRM ID"));
      vi.mocked(getRoadsideProductData).mockReturnValue(
      ...
      expect(redirectTo).toBe("/product-update-not-allowed");
      expect(mockRequestCookies.delete).toHaveBeenCalledTimes(1);
      expect(mockRequestCookies.delete).toHaveBeenCalledWith(uyvSessionIdCookieName);
      expect(createUpdateYourVehicleSession).not.toHaveBeenCalled();
      expect(mockRequestCookies.set).not.toHaveBeenCalled();
    ...
    it("should return redirectTo /product-update-not-allowed when canUpdateVehicle is false, reason is PRODUCT_NOT_ENABLED and roadside product line type is not OTHER", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      vi.mocked(cookies).mockResolvedValue(mockRequestCookies);
      vi.mocked(getCrmId).mockReturnValue(Promise.resolve("CRM ID"));
      vi.mocked(getRoadsideProductData).mockReturnValue(
    ...
    it("should return redirectTo /system-unavailable when canUpdateVehicle is false, reason is PRODUCT_NOT_ENABLED and roadside product line type is OTHER", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      vi.mocked(cookies).mockResolvedValue(mockRequestCookies);
      vi.mocked(getCrmId).mockReturnValue(Promise.resolve("CRM ID"));
      vi.mocked(getRoadsideProductData).mockReturnValue(
    ...
    it("should return redirectTo /system-unavailable when canUpdateVehicle is false and reason is null", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      vi.mocked(cookies).mockResolvedValue(mockRequestCookies);
      vi.mocked(getCrmId).mockReturnValue(Promise.resolve("CRM ID"));
      vi.mocked(getRoadsideProductData).mockReturnValue(
    ...
    it("should return redirectTo /system-unavailable when product holding line is missing vehicle details", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      const mockSessionId = crypto.randomUUID();
      const mockCrmId = crypto.randomUUID();
      const {
    ...
    it("should return redirectTo /system-unavailable when creating a session fails", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      const mockCrmId = crypto.randomUUID();
    ...
    it("should delete existing session cookie, create a new session, set the session ID cookie and return redirectTo /your-vehicle", async () => {
      const mockRequestCookies = mockReadonlyRequestCookies();
      const mockSessionId = crypto.randomUUID();
      const mockCrmId = crypto.randomUUID();
      const mockRoadsideProductData = mockGetRoadsideProductDataResponse();
      ...
      expect(redirectTo).toBe("/your-vehicle");
      expect(mockRequestCookies.delete).toHaveBeenCalledTimes(1);
      expect(mockRequestCookies.delete).toHaveBeenCalledWith(uyvSessionIdCookieName);
      expect(createUpdateYourVehicleSession).toHaveBeenCalledTimes(1);
      expect(createUpdateYourVehicleSession).toHaveBeenCalledWith<Parameters<typeof createUpdateYourVehicleSession>>({

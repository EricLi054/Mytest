...
describe("getVideoCarousel", () => {
  it("should call execute with correct query and variables", async () => {
    ...
    const result = await getVideoCarousel(mockId);
  ...
  it("should handle an empty response gracefully", async () => {
    ...
    const mockResponse = { horizons_videoCarousel: null };
  ...
  it("should throw an error if execute fails", async () => {
    ...
    await expect(getVideoCarousel(mockId)).rejects.toThrow("GraphQL error");
  });
});

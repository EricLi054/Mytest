...
describe("getArticlesFromCategory", () => {
  const mockCategory = "Drive";
  ...
  it("should call execute with correct query and variables", async () => {
    ...
    const result = await getArticlesFromCategory(mockCategory);
  ...
  it("should handle errors when execute fails", async () => {
    ...
    await expect(getArticlesFromCategory(mockCategory)).rejects.toThrow(errorMessage);

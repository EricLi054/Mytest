...
describe("CantFindYou", () => {
  it("should be able to render", () => {
    render(<CantFindYou />);

    const homeLink = screen.getByRole("link", { name: "Home" });
    const phoneLink = screen.getByRole("link", { name: "1300 045 617" });
    const racHomePageLink = screen.getByRole("link", { name: /RAC homepage/i });

    expect(screen.getByText("Sorry, we couldn't find you")).toBeVisible();
    ...
    expect(screen.getByText("If you're still having issues, please call us on", { exact: false })).toBeVisible();
    expect(homeLink).toBeVisible();
    expect(homeLink).toHaveAttribute("href", racHomePageUrl);
    expect(phoneLink).toBeVisible();
    expect(phoneLink).toHaveAttribute("href", "tel:1300045617");
